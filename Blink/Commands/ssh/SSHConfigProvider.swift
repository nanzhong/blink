...
class SSHClientConfigProvider {
  ...
  static func config(command cmd: SSHCommand, using device: TermDevice) -> (String, SSHClientConfig) {
    ...
    let port = BKConfig.port(forHost: host)
    
    return (
      BKConfig.hostName(forHost: host) ?? cmd.host,
      SSHClientConfig(
        // first use 'user' from options, then from cmd, then from configured host, and last defaultUserName and fallback to `root`
        user: options?.user ?? cmd.user ?? BKConfig.user(forHost: host) ?? BKDefaults.defaultUserName() ?? "root",
        // first use `port` from opts, then from cmd, then from configured host, and defaults to 22
